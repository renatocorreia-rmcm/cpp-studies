    ...
    void blit(){
        ...
        for (int x=0; x<height; x++){
            for (int y=0; y<width; y++) {
                cout << (screen[x * width + y] == logic_void_pixel ? blit_void_pixel : blit_full_pixel) << ' ';
            }
            cout << endl;
        }
    }
...
int main(){
    ...
    */
    ...
    // randomness and initials fruit_position
    srand(time(NULL));
    ...
    while (!is_game_over){
        ...
        if ((jobert.new_head%screen.width != jobert.past_head%screen.width && jobert.new_head/screen.width != jobert.past_head/screen.width)  // check hitted wall: x1!=x2 and y1!=y2
            || (jobert.new_head < 0 || jobert.new_head >= screen.size)  // check hitted ceilling
            || ((screen.screen[jobert.new_head]!=screen.logic_void_pixel) && (screen.screen[jobert.new_head]!=screen.logic_fruit_pixel) && (jobert.new_head!=jobert.new_tail))){  // check hitted itself
